...
def percentage_finder(all_proteins: pd.Series):
    ...
    for i, record in enumerate(records):
        ...
        if i % 100 == 0:
            ...
            with open(f'{base_path}/data/outfile.fasta', 'a', encoding='utf-8') as outfile:
        ...
        if all_proteins.str.contains(identifiers).any():
            print(all_proteins.str.get(identifiers) + '-' + identifiers)
            print(f'[{annotated_counter}/5609 encontrado!')
...
def main():
    with open(f'{base_path}/entire_output.txt', 'r') as f:
        lines = f.readlines()
