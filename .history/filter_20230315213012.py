...
def percentage_finder(identifiers: pd.Series, all_proteins: pd.Series, thread_id: int):
...
def main():
    ...
    records = SeqIO.parse(f'{base_path}/data/mtb_proteome_cat.fasta', 'fasta')
    descriptions = []
    [descriptions.append(rec.description) for rec in records]
    
    with open(f'{base_path}/data/entire_output.txt', 'r') as f:
        lines = f.readlines()

    chunks = np.array_split(np.array(descriptions), n_threads)
